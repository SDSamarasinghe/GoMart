...
const AdminFeedbackView  = () => {
  ...
  const deleteFeedbacks = (id) => {
    swal({
      title: "Are you sure?",
      text: "Once deleted, you will not be able to recover this Feedbacks!",
      icon: "warning",
      buttons: true,
      dangerMode: true,
    }).then((willDelete) => {
      if (willDelete) {
        axios
          ...
          .then(() => {
            swal("Feedbacks Deleted Successfully!", {
              icon: "success",
            });
  ...
  const printPdf = () => {
    ...
    html2canvas(input).then((canvas) => {
      ...
      doc.text(
        5,
        50,
        "Following are the feedbacks received currently."
      );
    ...
    <div className="store-add-product py-4 d-flex align-items-center flex-column justify-content-center">
      <div className=" store-form-outer-layer" style={{width:"1654.250px" ,height:"100px", marginBottom:"20px", padding:"25px" }}>
        ...
        <div  style={{display:"block", }}>
          <a href="/Message/AdminMessagesView" style={{fontSize:"40px" , color:"black" , paddingRight:"100px" , paddingLeft:"100px", fontWeight:"bolder" }}> Messages</a>
          <a href="/Feedbacks/AdminFeedbackView" style={{fontSize:"40px" , color:"black" , paddingRight:"100px" , paddingLeft:"100px" , fontWeight:"bolder" , borderBottom:"3px solid #248C78"}}>Feedbacks</a>
        ...
        <div className=" w-100" id="store-admin-admin456412123">
          ...
          <div className="d-flex">
            ...
            <button onClick={printPdf} className="btn btn-success mx-4">
              <i class="fa-solid fa-file-pdf mx-2"></i> Download Feedbacks
            </button>
          </div>
              ...
              {Feedbacks &&
                Feedbacks.map((fb,index) => (
                  <tr >
                    ...
                    <td style={{fontFamily:"sans-serif"}}>{fb.fname}</td>               
                <td style={{fontFamily:"sans-serif"}}> {fb.lname}</td>              
                <td style={{fontFamily:"sans-serif"}}>{fb.date}</td>
                <td style={{fontFamily:"sans-serif"}}>{fb.email}</td>
                <td style={{fontFamily:"sans-serif"}}>{fb.pro_qua_fb}</td>
                <td style={{fontFamily:"sans-serif"}}>{fb.deli_dri_rate}</td>
                <td style={{fontFamily:"sans-serif"}}>{fb.deli_tme_rate}</td>
                <td style={{fontFamily:"sans-serif"}}>{fb.recommendation}</td>
                <td style={{fontFamily:"sans-serif"}}>{fb.suggestions}</td>
                    <td style={{ width: "300px" }}>
                      <Link to={`/Feedbacks/AdminEditFeedback/${fb._id}/${fb.fname}/${fb.lname}/${fb.date}/${fb.email}/${
                      fb.pro_qua_fb}/${fb.deli_dri_rate}/${fb.deli_tme_rate}/${fb.recommendation}/${fb.suggestions}`}>
                        <button
                          type="button"
                      ...
                      <button
                        onClick={() => deleteFeedbacks(fb._id)}
                        type="button"
