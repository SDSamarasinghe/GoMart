...
const printPdf = () => {
  ...
  return (
    ...
    <div className="col-md-8 mt-4 mx-auto" style={{marginBottom:"20px"}}>
      ...
      <h3> <span className="secondary p-1 px-4 rounded text-white" style={{backgroundColor:"#5cb85c", fontSize:"50px"}}>{greet} Admin</span></h3>
            ...
            adsr.map((adr, index) => (
              <tr key={index}>
                <th scope="row" style={{fontFamily:"sans-serif", fontWeight:1000}}>
                  <Link
                    to={`/Feedbacks/AdminEditFeedback/${adr._id}/${adr.fname}/${adr.lname}/${adr.date}/${adr.email}/${
                ...
                <td style={{fontFamily:"sans-serif",width:"200px"}}>
                  <Link
                    ...
                    to={`/Feedbacks/AdminEditFeedback/${adr._id}/${adr.fname}/${adr.lname}/${adr.date}/${adr.email}/${
                      adr.pro_qua_fb}/${adr.deli_dri_rate}/${adr.deli_tme_rate}/${adr.recommendation}/${adr.suggestions}`}
                  >
                   {/* to={`/Feedbacks/EditFeedbackNew/${adr._id}`}
                  onClick={() => onUpdate(adr._id)} */}
