    ...
    swal({
      ...
      if (willDelete) {
        axios
          ...
          .then(() => {
            swal("Delivery Report Details Deleted Successfully!", {
              icon: "success",
            });
            refresh();
  ...
  return (
    ...
    <div className="col-md-8 mt-4 mx-auto" style={{marginBottom:"20px"}}>
     ...
     <Link to="Product/adform"><p style={{marginTop:"70px",color:"#ffffff",fontWeight:"bold"}}>Customers</p></Link>
      ...
      <table className="table table-hover pdfdiv" style={{ marginTop: "40px", width:"1000px", marginLeft:"130px"}}>
        <thead>
          ...
          <tr>
            ...
            <th scope="col">Special Notices</th>
        ...
        {adsr.map((adr,index) =>(
                            <tr>
                                ...
                                <td>{adr.driver_Name}</td>
                                <td>{adr.Receviver_Name}</td>
                                <td>{adr.delivery_date}</td>
                                <td>{adr.delivery_time}</td>
                                <td>{adr.delivery_status}</td>
                                <td>{adr.Receviver_phoneNo}</td>
                                <td>{adr.delivery_details}</td>
                                <td>{adr.special_Notices}</td>
                                <td>{adr.Date}</td>
                                <td>{adr.signature}</td>
                                <td>
                                    <Link to={{ 
                                    pathname: "/delivery/editdelivery/"+adr._id, 
                                    param1: "Par1" 
      ...
      <br />
      <button className="btn btn-warning" onClick={printPdf} style={{marginRight:"80px"}}>Download delivery Report</button>
